...
kubectl apply -f $CURRENT_DIR/third_party/cert-manager-latest/cert-manager.yaml
kubectl wait --for=condition=Established --all crd
kubectl wait --for=condition=Available -n cert-manager --all deployments

echo "Deploying Knative Serving..."
ko apply --selector knative.dev/crd-install=true -Rf $CURRENT_DIR/config/core/
kubectl wait --for=condition=Established --all crd
ko apply -Rf $CURRENT_DIR/config/core/

echo "Setting up sslip.io domain name"
ko delete -f $CURRENT_DIR/config/post-install/default-domain.yaml --ignore-not-found
ko apply -f $CURRENT_DIR/config/post-install/default-domain.yaml

echo "Deploying Knative ingress with Kourier..."
kubectl apply -f $CURRENT_DIR/third_party/kourier-latest/kourier.yaml
